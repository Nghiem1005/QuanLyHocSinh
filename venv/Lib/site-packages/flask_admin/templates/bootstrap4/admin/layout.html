  ...
  {%- for item in menu_root %}
    ...
    {%- else %}
      {%- if item.is_accessible() and item.is_visible() -%}
        ...
        {%- if item.is_active(admin_view) %}
          <li class="active{% if class_name %} {{ class_name }}{% endif %} text-center d-flex justify-content-center">
            {%- else %}
          <li{% if class_name %} class="{{ class_name }}"{% endif %} class="text-center d-flex justify-content-center">
        {%- endif %}
      <a class="nav-link pr-2 d-flex flex-column justify-content-center" href="{{ item.get_url() }}"{% if item.target %} target="{{ item.target }}"{% endif %}>
